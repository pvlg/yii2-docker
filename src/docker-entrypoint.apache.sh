...
if [[ "$1" = 'supervisor' ]]; then
    ...
    if [[ "${YII_APP_TEMPLATE}" = 'basic' ]]; then
        ...
        if [[ ! -f  /etc/apache2/sites-enabled/basic.conf ]]; then
            cp /etc/apache2/sites-enabled/basic.conf.tmpl /etc/apache2/sites-enabled/basic.conf
        fi
        ...
        sed -i "s@#APP_DOMAIN#@${APP_DOMAIN}@g" /etc/apache2/sites-enabled/basic.conf
        sed -i "s@#APP_PORT#@${APP_PORT}@g" /etc/apache2/sites-enabled/basic.conf
        sed -i "s@#APP_ROOT#@${APP_ROOT}@g" /etc/apache2/sites-enabled/basic.conf
    ...
    elif [[ "${YII_APP_TEMPLATE}" = 'none' ]]; then
        echo "Disabled template"
    fi
